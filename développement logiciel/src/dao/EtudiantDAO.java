 ...
 * @author Equipe7_4
 * @version finale
 * Classe permettant de connecter les étudiants de la BDD au code
 */
 ...
 */
	    ...
	    public List<String> getDominantesByUsername(String username) {
	        ...
	        String sql = "SELECT DOMINANTE FROM CHOICES WHERE USERNAME = ? ORDER BY ORDRE_CHOIX";
	        ...
	        try (Connection conn = ConnectionDAO.getConnection();
	            ...
	            while (rs.next()) {
	                dominantes.add(rs.getString("DOMINANTE"));
	            }
	    ...
	    public String getDominanteObtenue(String username) {
	        String dominante = null;
	        try {
	            String sql = "SELECT NOM_DOMIN FROM SAVE_CHOIX_ETU WHERE USERNAME = ?";
	            PreparedStatement stmt = connection.prepareStatement(sql);
	            stmt.setString(1, username);
	            ResultSet rs = stmt.executeQuery();
	            if (rs.next()) {
	                dominante = rs.getString("NOM_DOMIN");
	            }
	        } catch (SQLException e) {
	            e.printStackTrace();
	        }
